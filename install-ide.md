...
- [Создание первой программы](#создание-первой-программы)
  - [Организация файлов проектов](#организация-файлов-проектов)
  - [Visual Studio](#visual-studio)
  - [Rider](#rider)
  - [VS Code](#vs-code)
- [Запрет на top-level statements](#запрет-на-top-level-statements)
...
Связано это с тем, что англоязычное сообщество будет всегда больше, чем русскоязычное, просто потому что английский все еще остается международным языком и единственный способ контактировать людям из разных стран - это говорить на английском. Если искать ответ на вопрос на английском вы найдете ответ с большей вероятностью, чем на русском. Если пытаться переводить с русского на английский какие-то названия и термины, вы можете не угадать с переводом и не найдете нужный ответ. Если же сразу использовать английский, то вы точно не ошибетесь с термином.

Английский — must have в IT.

## Организация файлов проектов

До создания первого проекта выберите одну постоянную папку для учебных проектов и запомните, на каком диске и по какому пути она находится. Например:

```text
D:\StudentProjects
```

Каждый проект создавайте в отдельной папке внутри неё:

```text
D:\StudentProjects\Lesson01
D:\StudentProjects\Homework01
```

Не создавайте проекты в случайных папках, `Downloads` или на рабочем столе. Желательно не использовать в пути кириллицу и пробелы — это уменьшает количество проблем с инструментами разработки.

Вы должны уметь самостоятельно ответить на вопрос: **«На каком диске и в какой папке лежит мой проект?»** Если нужно открыть проект в проводнике, в Visual Studio или Rider нажмите правой кнопкой по проекту в `Solution Explorer` и выберите пункт открытия папки проекта.
...
Далее откроется окно, которое в дальнейшем и будет первым открываться при запуске VS. Это окно со списком недавно открывавшихся проектов (при вашем первом запуске будет пусто) и возможностью создать новый проект. Нажимаем `Create a new project`.

![alt](./img/vs-projects.png)

Нужно выбрать тип проекта, который нужен. Достаточно долгое время вы будете создавать только консольные приложения. Поэтому ищем тип `Console App`. Чтобы отфильтровать доступные варианты, можно воспользоваться выпадающими списками: выбрать язык C# и тип приложения `Console`. Нажимаем "Далее".

![alt](./img/vs-project-type.png)

Задайте осмысленное имя проекта на английском языке. В поле `Location` укажите заранее выбранную постоянную папку для проектов, например `D:\StudentProjects`. Не оставляйте путь по умолчанию, если не понимаете, где он находится: вы должны знать, на каком диске и в какой папке будет создан проект.

На следующем экране дополнительных параметров **установите** флажок `Do not use top-level statements`. В русской локализации этот пункт может называться «Не использовать инструкции верхнего уровня». Это сразу создаст классическую структуру с классом `Program` и методом `Main`, которую мы используем на занятиях. После этого нажимаем "Далее".

![alt](./img/vs-project-name.png)

Проект создан. VS открывает окно редактора, в котором можно будет в дальнейшем писать код. Справа будет Solution Explorer, где в виде дерева будет видна структура вашего проекта со всеми подключенными файлами. Проверьте расположение проекта: нажмите правой кнопкой по проекту в `Solution Explorer` и выберите `Open Folder in File Explorer`.
...
## Rider
При самом первом запуске Rider попросит выбрать начальные настройки. Если хотите, можете поменять под себя, например, цветовую тему, или оставить все настройки без изменений и прокликать вперед до экрана с проектами.

При старте Rider открывается экран с недавно открывавшимися проектами. При первом запуске он будет пустой. Также на этом экране будет кнопка `New Solution` для создания нового проекта. Кликаем её.
...
Задайте осмысленное имя проекта на английском языке. В поле пути укажите заранее выбранную постоянную папку для проектов, например `D:\StudentProjects`. Не оставляйте путь по умолчанию, если не понимаете, где он находится: вы должны знать, на каком диске и в какой папке будет создан проект.

Установите флажок `Do not use top-level statements`. В русской локализации этот пункт может называться «Не использовать инструкции верхнего уровня». Тогда Rider сразу создаст классическую структуру с классом `Program` и методом `Main`, которую мы используем на занятиях. После этого нажимаем "Далее".

![alt](./img/rider-new-project.png)

Проект создан. Откроется окно редактора, в котором можно будет в дальнейшем писать код. Слева будет Solution Explorer, где в виде дерева будет видна структура вашего проекта со всеми подключенными файлами. Проверьте расположение проекта: нажмите правой кнопкой по проекту в `Solution Explorer` и выберите `Open Folder in File Explorer`.
...
Сначала в заранее выбранной папке для учебных проектов создайте отдельную папку, в которой будет находиться программа. Например: `D:\StudentProjects\TestProject`. Дайте папке осмысленное название и запомните, где она находится.
...
Замените текст в файле `launch.json` на следующий:
```json
{
    "version": "0.2.0",
    "configurations": [
...
При типе запуска `dotnet` и передаче файла проекта у вас на самом деле выполняются два действия: сборка проекта и запуск. Но тип запуска `coreclr` не умеет сам выполнять сборку — он запускает то, что уже было собрано. Поэтому появилась дополнительная настройка, в частности `"preLaunchTask": "build"`. Она говорит, какую задачу нужно выполнить перед запуском проекта. Здесь и должна быть настроена сборка.

Нужно создать рядом с `launch.json` еще один файл `tasks.json` с конфигом:
...
Опять же обратите внимание, что в `tasks.json` указывается путь до проекта, а не до скомпилированной DLL внутри папки `bin`.
...
# Запрет на top-level statements

В C# у программы должна быть одна точка входа. В классическом варианте она выглядит так:
...
Top-level statements удобны для коротких примеров, но скрывают от начинающего разработчика класс `Program` и метод `Main`. Поэтому в рамках этого курса верхнеуровневые инструкции не используются: всегда пишите полную запись класса `Program` и метода `Main`.
...
По умолчанию Visual Studio и Rider создают проекты именно с верхнеуровневыми конструкциями. Поэтому после создания проекта перепишите файл `Program.cs`.

В Visual Studio Code можно сразу создать файл без этих конструкций, если использовать флаг `--use-program-main`.
