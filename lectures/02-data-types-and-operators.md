...
Когда в программе появляется ссылочная переменная, связь можно показать стрелкой от области хранения к объекту. На общей схеме стрелка не означает, что `serverName` — это сам текст. Она показывает ссылку на объект, где текст размещён. Для `int players` стрелка к отдельному объекту не нужна: в учебной модели число находится прямо в области переменной.
...
Для первых лабораторных достаточно модели, показанной на общей схеме выше. Это отличается от `int`, где переменная содержит само числовое значение. В случае строки переменная обычно содержит ссылку на объект, а сам текст находится в другом месте памяти. Подробное различие между значениями и ссылками мы разберём позже на классах и объектах.
