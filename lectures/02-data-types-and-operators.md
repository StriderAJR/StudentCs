...
![Переменные как области хранения](../img/lectures/02/stack-heap-reference-types.png)
...
![Значимые и ссылочные переменные в стеке и куче](../img/lectures/02/stack-heap-reference-types.png)

На схеме `string ref` и `Human ref` — это не настоящие имена типов C#, а наша учебная пометка. В коде типы по-прежнему называются `string` и `Human`; слово `ref` напоминает, что в ячейке переменной находится ссылка на объект.

![Стек и куча](../img/lectures/02/stack-heap-reference-types.png)

Когда в программе появляется ссылочная переменная, связь можно показать стрелкой от области хранения к объекту:

![Связь переменной с объектом](../img/lectures/02/stack-heap-reference-types.png)
...
![Ссылка на объект строки](../img/lectures/02/stack-heap-reference-types.png)
