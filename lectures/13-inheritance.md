...
Объект наследника содержит части базового и производного типа в одном объекте кучи. На схеме выше это один объект `Player`: внутри него находятся унаследованные поля `X`, `Y` и собственное поле `Health`.
