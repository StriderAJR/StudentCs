...
internal static class ConsoleFontHelper
{
    ...
    [DllImport("kernel32.dll", SetLastError = true, CharSet = CharSet.Unicode)]
    private static extern bool SetCurrentConsoleFontEx(IntPtr hConsoleOutput, bool bMaximumWindow, ref CONSOLE_FONT_INFO_EX lpConsoleFontEx);
    ...
    // Attempt to read the current console font face name. Returns null on failure.
    public static string? GetCurrentFontFaceName()
    {
        try
        {
            IntPtr h = GetStdHandle(STD_OUTPUT_HANDLE);
            var info = new CONSOLE_FONT_INFO_EX();
            info.cbSize = (uint)Marshal.SizeOf<CONSOLE_FONT_INFO_EX>();
            if (GetCurrentConsoleFontEx(h, false, ref info))
            {
